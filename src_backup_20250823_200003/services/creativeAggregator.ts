...
import { MetaInsightsData } from '../features/meta-api/core/types'
...
export class CreativeAggregator {
  ...
  async aggregateCreatives(
    ...
    if (options.campaignIds && options.campaignIds.length > 0) {
      filteredMetrics = filteredMetrics.filter(
        (metric) => metric.campaign_id && options.campaignIds!.includes(metric.campaign_id)
      )
    }
  ...
  */
    ...
    periodGroups.forEach((periodInsights, periodKey) => {
      ...
      creativeGroups.forEach((creativeInsights, creativeId) => {
        ...
        const aggregatedMetric: CreativeMetrics = {
          ...
          video_url:
            typeof creativeDetail.video_url === 'string'
              ? creativeDetail.video_url
              : typeof firstInsight.video_url === 'string'
                ? firstInsight.video_url
                : undefined,
          carousel_cards: Array.isArray(creativeDetail.carousel_cards)
            ? creativeDetail.carousel_cards
            : Array.isArray(firstInsight.carousel_cards)
              ? firstInsight.carousel_cards
              : undefined,
          campaign_id:
            typeof firstInsight.campaign_id === 'string'
              ? firstInsight.campaign_id
              : typeof firstInsight.campaignId === 'string'
                ? firstInsight.campaignId
                : undefined,
          campaign_name:
            typeof firstInsight.campaign_name === 'string'
              ? firstInsight.campaign_name
              : typeof firstInsight.campaignName === 'string'
                ? firstInsight.campaignName
                : undefined,
   ...
   * クリエイティブごとにグループ化
   */
  private groupByCreative(insights: MetaInsightsData[]): Map<string, MetaInsightsData[]> {
    const groups = new Map<string, MetaInsightsData[]>()
   ...
   * クリエイティブタイプを判定
   */
  private determineCreativeType(creativeDetail: any, insight: MetaInsightsData): CreativeType {
