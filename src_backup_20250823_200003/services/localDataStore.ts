...
class LocalDataStore extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      creatives:
        '++_localId, id, accountId, [accountId+id], creative_type, fetchedAt, [dateRange.since+dateRange.until]',
      insights:
        '++_localId, [accountId+date_start+campaign_id], accountId, campaign_id, ad_id, creative_id, fetchedAt',
  ...
  async saveCreatives(
    ...
    await this.creatives
      ...
      .and(
        (item) =>
          item.dateRange.since === dateRange.since && item.dateRange.until === dateRange.until
      )
      .delete()
  ...
  async cleanup(daysToKeep: number = 30) {
    ...
    const creativesDeleted = await this.creatives.where('fetchedAt').below(cutoffDate).delete()

    const insightsDeleted = await this.insights.where('fetchedAt').below(cutoffDate).delete()
