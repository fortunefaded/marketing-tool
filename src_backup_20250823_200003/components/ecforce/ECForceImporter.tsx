...
export const ECForceImporter: React.FC = () => {
  ...
  const {
    ...
    loadMore,
    hasMore,
  } = useECForceDataPaginated({ pageSize: 100 })
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-gray-900 mb-8">EC Force データインポート</h1>
      ...
      {importProgress.status === 'complete' && (
        <div className="mt-4 bg-green-50 border border-green-200 rounded-md p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-green-800">インポートが完了しました</p>
              {duplicateStats && duplicateStats.skipped + duplicateStats.replaced > 0 && (
                <div className="mt-2 text-sm text-green-700">
                  <p>• 新規インポート: {duplicateStats.imported}件</p>
                  {duplicateStats.skipped > 0 && <p>• スキップ: {duplicateStats.skipped}件</p>}
                  {duplicateStats.replaced > 0 && <p>• 置き換え: {duplicateStats.replaced}件</p>}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mt-8">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-900">
              データサマリー（{totalCount ? `全${totalCount.toLocaleString()}件中` : ''}
              {importedData.length.toLocaleString()}件表示）
            </h2>
            <button
              onClick={async () => {
      ...
      {importedData.length > 0 && (
        <div className="mt-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            インポートデータ ({importedData.length.toLocaleString()}件 /{' '}
