...
export const RFMAnalysis: React.FC<RFMAnalysisProps> = ({ orders }) => {
  ...
  const rfmData = useMemo(() => {
    ...
    const customers: CustomerRFM[] = Object.entries(customerData).map(([customerId, data]) => {
      const recency = Math.floor(
        (now.getTime() - data.lastPurchase.getTime()) / (1000 * 60 * 60 * 24)
      )
  ...
  return (
    <div className="space-y-6">
            ...
            <ScatterChart margin={{ top: 20, right: 20, bottom: 60, left: 60 }}>
              ...
              <Scatter name="顧客" data={scatterData} fill="#8884d8">
                {scatterData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={segmentStats[entry.segment]?.color || '#6B7280'}
                  />
                ))}
              </Scatter>
            </ScatterChart>
      ...
      {/* セグメント別アクション提案 */}
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">セグメント別マーケティング施策</h3>
        <div className="space-y-4">
          <div className="border-l-4 border-green-500 pl-4">
            <h4 className="font-medium text-gray-900">
              チャンピオン（{segmentStats['チャンピオン'].count}人）
            </h4>
          ...
          <div className="border-l-4 border-blue-500 pl-4">
            <h4 className="font-medium text-gray-900">
              優良顧客（{segmentStats['優良顧客'].count}人）
            </h4>
            <p className="text-sm text-gray-600 mt-1">
          ...
          <div className="border-l-4 border-indigo-500 pl-4">
            <h4 className="font-medium text-gray-900">
              潜在的優良顧客（{segmentStats['潜在的優良顧客'].count}人）
            </h4>
            <p className="text-sm text-gray-600 mt-1">
          ...
          <div className="border-l-4 border-purple-500 pl-4">
            <h4 className="font-medium text-gray-900">
              新規顧客（{segmentStats['新規顧客'].count}人）
            </h4>
            <p className="text-sm text-gray-600 mt-1">
          ...
          <div className="border-l-4 border-yellow-500 pl-4">
            <h4 className="font-medium text-gray-900">
              離反リスク（{segmentStats['離反リスク'].count}人）
            </h4>
            <p className="text-sm text-gray-600 mt-1">
          ...
          <div className="border-l-4 border-red-500 pl-4">
            <h4 className="font-medium text-gray-900">
              休眠顧客（{segmentStats['休眠顧客'].count}人）
            </h4>
            <p className="text-sm text-gray-600 mt-1">
      ...
      {/* 顧客詳細テーブル */}
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">顧客詳細</h3>
        ...
        <VirtualizedTable
          ...
          columns={[
            ...
            },
            {
              ...
              header: 'RFMスコア',
              width: 100,
              render: (customer) => <span className="font-mono text-sm">{customer.score}</span>,
            },
          ]}
          rowHeight={48}
          height={400}
