    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex justify-between">
        ...
        <button
          ...
          disabled={currentStep === 'permissions' ? !requiredPermissionsGranted : !isTestComplete}
          className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white ${
            (currentStep === 'permissions' && requiredPermissionsGranted) || (currentStep === 'test' && allTestsPassed)
              ? 'bg-indigo-600 hover:bg-indigo-700'
              : 'bg-gray-400 cursor-not-allowed'
          }`}
        >
          {currentStep === 'permissions' ? 'テストを実行' : allTestsPassed ? '次へ進む' : 'エラーを解決してください'}
        </button>
      </div>
        </div>
      </div>
