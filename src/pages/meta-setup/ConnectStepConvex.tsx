...
export const ConnectStepConvex: React.FC = () => {
  ...
  const handleAddOrUpdateAccount = async () => {
    ...
    try {
      ...
      if (editingAccount) {
        // 既存アカウントの更新
        await manager.updateAccount(editingAccount.accountId, {
          name: newAccountForm.name || editingAccount.name,
  ...
  const handleEditAccount = (account: MetaAccount) => {
  ...
  const handleSelectAccount = async (account: MetaAccount) => {
    try {
      console.log('Setting active account:', account.accountId)
  ...
  const handleContinue = () => {
    if (selectedAccount) {
      navigate('/settings/meta-api/permissions')
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Meta広告アカウントの接続</h1>
      ...
      {showAddForm && (
        <div className="border-t pt-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center">
            ...
            {editingAccount ? 'アカウント情報の編集' : '新規アカウントの追加'}
          </h3>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                ...
                {isConnecting ? '保存中...' : editingAccount ? '変更を保存' : 'アカウントを追加'}
              </button>
            </div>
          </div>
        </div>
