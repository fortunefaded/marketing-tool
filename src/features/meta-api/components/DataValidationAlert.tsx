...
export function DataValidationAlert({
  ...
  const validateData = (): ValidationIssue[] => {
    ...
    const instagramAds = data.filter(
      (ad) =>
    ...
    if (instagramAds.length > 0) {
      const noEngagementAds = instagramAds.filter(
        (ad) => ad.metrics.instagram_metrics.engagement_rate === 0
      )
  ...
  if (
    ...
    (validationIssues.length === 1 && validationIssues[0].type === 'info' && data.length > 0)
  ) {
    return (
      ...
      <div className="p-3">
          ...
          <div className="flex items-center">
            ...
            <span
              className={`text-sm font-medium ${
                hasErrors ? 'text-red-800' : hasWarnings ? 'text-yellow-800' : 'text-blue-800'
              }`}
            >
        ...
        {isExpanded && (
          <div className="mt-3 space-y-2">
                  ...
                  <div className="mt-1">
                    ...
                    {issue.affectedAds.length > 0 && (
                      <div className="text-gray-600">
                        ...
                        {issue.count > issue.affectedAds.length &&
                          ` 他${issue.count - issue.affectedAds.length}件`}
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className={`p-2 rounded text-xs ${hasErrors ? 'bg-red-100' : 'bg-yellow-100'}`}>
              <strong>推奨アクション:</strong>
              <ul className="mt-1 list-disc list-inside space-y-1">
                {hasErrors && (
                  <li>
                    エラーが検出されました。「データ再取得」を試すか、Meta
                    APIの設定を確認してください。
                  </li>
                )}
                ...
                <li>
                  数値の差異がある場合は、データ更新のタイミングやAPIの更新遅延を考慮してください。
                </li>
              </ul>
            </div>
          </div>
