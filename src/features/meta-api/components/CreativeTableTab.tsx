...
export function CreativeTableTab({
  ...
  const sortedData = useMemo(() => {
    ...
    const enrichedData = data.map(item => {
      ...
      return {
        ...
        cpa: (item.metrics.conversions || 0) > 0 ? (item.metrics.spend || 0) / (item.metrics.conversions || 1) : 0,
        roas: insight?.conversion_value && item.metrics.spend > 0 ? insight.conversion_value / item.metrics.spend : 0,
    ...
    console.log('sortedData result:', { 
      ...
      sortField,
      sortDirection,
      firstValue: sortedItems[0]?.[sortField as keyof typeof sortedItems[0]] || (sortedItems[0]?.metrics as any)?.[sortField] || sortedItems[0]?.score,
      lastValue: sortedItems[sortedItems.length - 1]?.[sortField as keyof typeof sortedItems[0]] || (sortedItems[sortedItems.length - 1]?.metrics as any)?.[sortField] || sortedItems[sortedItems.length - 1]?.score
    })
  ...
  console.log('CreativeTableTab rendered:', { dataLength: data?.length, insightsLength: insights?.length, isLoading })
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full divide-y divide-gray-200 table-fixed" style={{ minWidth: '1560px' }}>
            <thead className="bg-gray-50">
              <tr>
                <th className="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" 
                    style={{ width: '80px' }}
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" style={{ width: '300px' }}
                ...
                <th className="px-2 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" style={{ width: '80px' }}
                ...
                <th className="px-2 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" style={{ width: '70px' }}
                ...
                <th className="px-2 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" style={{ width: '100px' }}
                ...
                <th className="px-2 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" style={{ width: '75px' }}
                ...
                <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '90px' }}>
                  ステータス
                </th>
                <th className="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '80px' }}>
                  詳細
                </th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {sortedData.map((item, index) => (
                <tr key={`${item.adId}-${index}`} className="hover:bg-gray-50">
                  {/* クリエイティブタイプ */}
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900 truncate" title={item.adName || `Creative ${index + 1}`}>
                  ...
                  <td className="px-2 py-3 whitespace-nowrap text-right text-sm">
                    <span className={item.metrics.frequency > 3.5 ? 'text-red-600 font-medium' : 'text-gray-900'}>
                  ...
                  {/* ROAS */}
                  <td className="px-2 py-3 whitespace-nowrap text-right text-sm">
                    <span className={item.roas >= 3.0 ? 'text-green-600 font-medium' : 
                                   item.roas >= 2.0 ? 'text-yellow-600' : 
                                   item.roas > 0 ? 'text-red-600' : 'text-gray-900'}>
                  ...
                  <td className="px-2 py-3 whitespace-nowrap text-center">
                    <button
                      ...
                      className="inline-flex items-center justify-center p-1.5 text-sm text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors duration-200"
                      title="詳細を表示"
                    >
        ...
        <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">
                {formatDecimal(sortedData.reduce((sum, item) => sum + (item.roas || 0), 0) / sortedData.length)}x
              </p>
            </div>
          </div>
        </div>
